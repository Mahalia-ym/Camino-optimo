...
class RyA(Queue):
    ...
    #Aquí está la diferencia, porque tenemos que añadir y reordenamos
    #toda la lista abierta por el pathcost
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda a:a.path_cost) #Aquí estamos ordenando
...
class RyASub(Queue):
    """Ramificacion y acotacion subestimada"""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda a:(a.path_cost+self.problem.h(a)))
